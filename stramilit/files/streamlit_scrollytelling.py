"""
=================================================================================
ANÁLISE DE SENTIMENTO - VERSÃO CORRIGIDA (Formatação de Textos)
=================================================================================
CORREÇÃO: Removidos texttemplate problemáticos, usando text + textposition
=================================================================================
"""
...
def carregar_dados():
    ...
    # Limpeza de strings (remover espaços)
...
def plot_classificacao_dinamica(class_counts: pd.Series):
    ...
    fig.add_trace(
        go.Bar(
            ...
            showlegend=False,
            text=[f"{v:,}" for v in class_counts.values],  # Formatação manual
            textposition='auto'
        ),
        row=1, col=1
    )
...
def plot_top_categorias(df_input, classificacao='Todas', top_n=10):
    ...
    # Usar Graph Objects em vez de Express
    fig = go.Figure()
    ...
    fig.add_trace(go.Bar(
        ...
        orientation='h',
        marker=dict(
            color=top_cat['Contagem'],
            colorscale='Viridis',
            showscale=True
        ),
        text=[f"{v:,}" for v in top_cat['Contagem']],  # Formatação manual
        textposition='auto',
...
def plot_visualizacao_cauda_longa(df_input):
    ...
    fig.add_vline(x=20, line_dash="dash", line_color="#2c3e50",
                  annotation_text="← Top 20 | Cauda Longa →", annotation_position="top right")
...
def plot_matriz_espacial(df_input, min_n=1):
    ...
    fig.update_layout(
        title=f"<b>Matriz Espacial: Sentimento × Volume (Mínimo {min_n} menções)</b>",
        xaxis_title="Volume Absoluto (Barulho)", 
        yaxis_title="NSS (Sentimento)",
        template="plotly_white", 
        height=600
    )
...
def plot_piramides_veiculo_absoluto(df_input: pd.DataFrame, top_n: int = 20):
    """Pirâmides Top Positivos/Negativos"""
    ...
    fig = make_subplots(
        rows=1, cols=2, 
        subplot_titles=(f'Top {top_n} NEGATIVOS', f'Top {top_n} POSITIVOS')
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color='#e74c3c', 
            showlegend=False,
            text=[f"{v:,}" for v in top_neg['Total']],
            textposition='auto',
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color='#2ecc71', 
            showlegend=False,
            text=[f"{v:,}" for v in top_pos['Total']],
            textposition='auto',
            hovertemplate='<b>%{y}</b><br>Total: %{x:,}<extra></extra>'
        ), 
        row=1, col=2
    ...
    fig.update_layout(
        title=f'<b>Pirâmides: Top {top_n} Veículos</b>', 
        template='plotly_white', 
...
def plot_heatmap_temporal_categoria(df_input: pd.DataFrame, top_n: int = 10, min_monthly: int = 3):
    ...
    fig.update_layout(
        title='<b>Heatmap Temporal: NSS × Categoria × Mês</b>', 
        template='plotly_white', 
        height=500, 
...
def main():
    ...
    # HERO
    st.markdown("""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 3rem 2rem; border-radius: 12px; color: white; text-align: center; margin-bottom: 2rem;">
        <h1 style="font-size: 2.5rem; margin-bottom: 0.5rem;">📊 Análise de Sentimento de Mídia</h1>
        <p style="font-size: 1.2rem; opacity: 0.9;">Dashboard completo com visualizações corrigidas</p>
    </div>
    """, unsafe_allow_html=True)
    
    # GRÁFICOS
    st.markdown('<div class="section-title">1. Distribuição de Sentimentos</div>', unsafe_allow_html=True)
    df_org_graf = df_filtrado[df_filtrado['Classificação'] != 'PUBLICIDADE']
    st.plotly_chart(plot_classificacao_dinamica(df_org_graf['Classificação'].value_counts()), use_container_width=True)
    
    st.markdown('<div class="section-title">2. Top Categorias</div>', unsafe_allow_html=True)
    col1, col2 = st.columns([1, 4])
    with col1:
        classif = st.selectbox("Sentimento", ['Todas', 'POSITIVA', 'NEUTRA', 'NEGATIVA'])
        top_n_cat = st.slider("Top N", 5, 20, 10, key='top_cat')
    with col2:
        st.plotly_chart(plot_top_categorias(df_filtrado, classif, top_n_cat), use_container_width=True)
    ...
    min_n = st.slider("Mínimo de menções", 1, 20, 5, key='matriz')
    ...
    top_pir = st.slider("Top N veículos", 5, 30, 20, key='piramides')
    ...
    min_tree = st.slider("Mín. menções", 5, 20, 10, key='treemap')
    ...
    top_heat = st.slider("Top N categorias", 5, 15, 10, key='heatmap')
